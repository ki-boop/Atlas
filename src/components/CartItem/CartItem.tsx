...
import { useDeleteFromCart } from "../../Context/UserContext/CartContext";
...
export const CartItem = (props: cartItem) => {
  ...
  return (
    <div className={"cart-item"}>
      <div className={"col"}>
        <Image name={props.img[0]} alt={props.strings.name} />
      </div>
      <div className={"col"}>{props.count}</div>
      <div className={"col"}>{props.strings.name}</div>
      <div className={"col"}>
        <Price price={props.strings.price} />
      </div>
      <div className={"col"}>{props.strings.type}</div>
      <div className={"col"}>
        <Button onClick={deleteItem}>{strings.delete}</Button>
      </div>
    </div>
  );
};
